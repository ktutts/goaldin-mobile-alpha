...
export default function Today(){
 ...
 const load=useCallback(async()=>{setLoading(true); const {data:{user}}=await supabase.auth.getUser(); if(!user){router.replace('/');return;} const [g,a]=await Promise.all([supabase.from('goals').select('*').eq('user_id',user.id).order('created_at',{ascending:false}),supabase.from('actions').select('*').eq('user_id',user.id).order('position')]); setGoals((g.data||[]) as Goal[]); setActions((a.data||[]) as Action[]); setLoading(false);},[]);
 useEffect(()=>{load()},[load]);
 async function done(a:Action){await supabase.from('actions').update({status:'completed',completed_at:new Date().toISOString()}).eq('id',a.id); await supabase.from('events').insert({user_id:a.user_id,event_type:'action_completed',object_id:a.id,metadata:{goal_id:a.goal_id}}); const remaining=actions.filter(x=>x.goal_id===a.goal_id&&x.status==='pending'&&x.id!==a.id); if(remaining.length===0){const goal=goals.find(g=>g.id===a.goal_id); if(goal){await supabase.from('goals').update({status:'completed'}).eq('id',goal.id); await supabase.from('wins').insert({user_id:a.user_id,goal_id:goal.id,title:goal.title,summary:'Completed in GOAL\'D IN'});}} await load();}
