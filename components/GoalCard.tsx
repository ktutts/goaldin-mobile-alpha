...
export default function GoalCard({ goal, actions, onDone }: { goal: Goal; actions: Action[]; onDone: (a: Action) => void }) {
  ...
  const next = actions.find(a => a.status === 'todo');
  const progress = Math.round((done / total) * 100);

  return <View style={s.card}>
    <View style={s.row}><Text style={s.title}>{goal.title}</Text><Text style={s.progress}>{progress}%</Text></View>
    <View style={s.track}><View style={[s.fill, { width: `${progress}%` }]} /></View>
    {next ? <>
      <Text style={s.label}>NEXT MOVE</Text>
      <Text style={s.next}>{next.title}{next.estimate_minutes ? ` · ${next.estimate_minutes} min` : ''}</Text>
      <Pressable style={s.button} onPress={() => onDone(next)}><Text style={s.buttonText}>DONE ✓</Text></Pressable>
...
const s = StyleSheet.create({
  ...
  button:{backgroundColor:'#D8B24A',padding:14,borderRadius:14,alignItems:'center'}, buttonText:{fontWeight:'900',color:'#0a0a0a'}, complete:{color:'#D8B24A',fontSize:18,fontWeight:'900'}
});
